...
class Direction {
  ...
  factory Direction.fromJson(Map<String, dynamic> json) {
    // print(json.keys);
    // final steps = json['routes'][0]['legs'][0]['steps'] as List<dynamic>;
    final polylinePoints = json['routes'][0]['polyline']['encodedPolyline'];

    return Direction(
      route: _decodePoly(polylinePoints),
      distanceText:
          (json['routes'][0]['distanceMeters'] / 1000).toStringAsFixed(1),
      durationText: (int.parse(
                json['routes'][0]['duration'].replaceAll(RegExp(r'[^0-9]'), ''),
              ) /
              60)
          .round()
          .toString(),
